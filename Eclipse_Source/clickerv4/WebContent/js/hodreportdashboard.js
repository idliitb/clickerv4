...
function loadCoursesData(insid){
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			var resp = xmlhttp.responseText.split("~&~");
			var mydashboardtable="";
			for(var i=0;i<resp.length-1;i++){
				var cid=resp[i].split("~^~")[0].trim();
				var courseresponse=resp[i].split("~^~")[1];
				mydashboardtable="<table class='table1' style='margin-top: 10px;'>" +
						"<tr>" +
						"<td>" +
							"<div class='ui-header-text' style='text-align:left;'>" +
								"<h2>Course ID : "+cid+"</h2>" +
							"</div>" +
							"<div class='notebox' onclick='changeActive(\""+cid+"nq\", \""+cid+"\", \""+insid+"\", \""+courseresponse.split("~$~")[1].split("@#@")[0]+"\")'>" +
								"<div id='"+cid+"nq_head' class='boxhead'>Normal quiz</div>"+
								"<div id='"+cid+"nq' class='boxnote'>00</div>"+
							"</div>" +
							"<div class='notebox' onclick='changeActive(\""+cid+"iq\", \""+cid+"\", \""+insid+"\", \""+courseresponse.split("~$~")[1].split("@#@")[1]+"\")'>"+
								"<div id='"+cid+"iq_head' class='boxhead'>Instant quiz</div>"+
								"<div id='"+cid+"iq' class='boxnote'>00</div>"+
							"</div>" +
							"<div class='notebox' onclick='changeActive(\""+cid+"p\", \""+cid+"\", \""+insid+"\", \""+courseresponse.split("~$~")[1].split("@#@")[3]+"\")'>"+
								"<div id='"+cid+"p_head' class='boxhead'>Poll</div>"+
								"<div id='"+cid+"p' class='boxnote'>00</div>"+
							"</div>" +
							"<div class='notebox' onclick='changeActive(\""+cid+"ts\", \""+cid+"\", \""+insid+"\", \""+courseresponse.split("~$~")[1].split("@#@")[2]+"\")'>"+
								"<div id='"+cid+"ts_head' class='boxhead'>Total Student</div>"+
								"<div id='"+cid+"ts' class='boxnote'>00</div>"+
							"</div>" +
						"</td>"+
						"<td rowspan='2' align='center' style='width: 490px;'>"+
							"<div id='"+cid+"qpChart' style='margin: auto; width:450px;height:170px;><img src='../../img/loading.gif'/></div>"+
							"<div id='"+cid+"qpChart1' ></div>" +
						"</td>" +
						"</tr>"+
						"<tr id='"+cid+"_arrowcontainer'></tr>"+
						"<tr id='"+cid+"_contentcontainer'></tr>"+
					"</table>";											
				document.getElementById("HODDashboard").innerHTML+=mydashboardtable;
				quizData(courseresponse,insid, cid);
			}
			arrowcontainer="<td><div class='downarrow_div'>"+
			"<img id='nq_arrow' style='visibility: visible;' class='downarrow' src='../../img/downarrow.png'></img>"+			
