...
function quizData(insid){	
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			//alert(JSON.stringify(nqseriesdata));
			...
			for(var i=1;i<quizdetails.length;i++){
				...
				//alert(timestamps[4] + " / " + quizcount[1]);
				quizzes += "<div class='mybox'><div class='myboxhead1'>"+timestamps[0] + " - " + timestamps[1] +"</div>" ;
				quizzes += "<div class='myboxnote' onclick='studentNormalQuizReport(\""+timestamps[0]+"\",\""+insid+"\")'>"+timestamps[2]+"</div>";
				quizzes += "<div class='myboxnote' onclick='studentInstantQuizReport(\""+timestamps[0]+"\",\""+insid+"\")'>"+timestamps[3]+"</div>";
			...
			for(var i=1;i<quizdetails.length;i++){
				...
				for(var j=2;j<resp.length;j++){
					if(resp[j].indexOf("Yes")!=-1 || resp[j].indexOf("No :")!=-1){
						YesNo += resp[j] + ", ";
					}else{
				...
				if(YesNo != ""){
					quizzes += "<div class='pollboxnote'>"+YesNo+"</div>";
				}
				quizzes += "</div>";
			}
	...
	xmlhttp.open("GET", "../../jsp/dashboard/reportdashboardhelper.jsp?helpcontent=quizdata", true);
	xmlhttp.send();	
}
...
function generatePollReport(pid){
	...
	xmlhttp.open("GET", "../../Report?report=pollreport&pid="+ pid, true);
	xmlhttp.send();
}

function studentNormalQuizReport(sid, iid){
	$("#tempdiv").load("../../studentPerformanceChart?sid="+sid+ "&qtype=nquiz", function(){
		$("#dlg_body").load("../../Report?sid="+sid + "&report=studreport", function(){
...
function studentInstantQuizReport(sid, iid){
	$("#tempdiv").load("../../studentPerformanceChart?sid="+sid + "&qtype=iquiz", function(){
		$("#dlg_body").load("../../Report?sid="+sid + "&report=studreport", function(){
...
function fillAttenDetail(courseID, atdates, date) {
	...
	var dateArray = date.split("/");
	date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	xmlhttp.open("GET",	"../../jsp/dashboard/reportdashboardhelper.jsp?helpcontent=atteninfo&cid="+ encodeURIComponent(courseID) + "&date="+date, true);	
	xmlhttp.send();	
}
