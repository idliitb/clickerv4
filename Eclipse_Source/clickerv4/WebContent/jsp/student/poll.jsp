...
String courseId=polldetailsarray[2]; 
...
Date d2 = null;
	...
	{
		//time= pollt;
		newpolldetails=sid+"@"+courseId+"@"+quizTime+"@"+pollid;
	}
	else
	{
		time = pollt-(diff_sec);
		...
		newpolldetails=sid+"@"+courseId+"@"+newpolltime+"@"+pollid;
	}
...
var mode="local";
	function showpollquestion(pollquestion,newpolldetails)	{
				...
				studentid=polldetails[0];
				courseid=polldetails[1];
	...
	function Decrement()
	{
		...
		var response='poll_res={"stuid":'+studentid+',"option":'+ans+'}&courseId='+courseid+'&mode='+mode+'&pollId='+pollid;
		setPollResopnseCookie("previouspollresponse",response,7);
	...
	function sendans(){
		...
		xmlhttp1.onreadystatechange = function() {
			...
			if (xmlhttp1.readyState == 4 && xmlhttp1.status == 200) {
				...
				setCookie("polllastattempted",pollid, 7);
		...
		xmlhttp1.send('poll_res={"stuid":'+studentid+',"option":'+ans+'}&courseId='+courseid+'&mode='+mode+'&pollId='+pollid);
