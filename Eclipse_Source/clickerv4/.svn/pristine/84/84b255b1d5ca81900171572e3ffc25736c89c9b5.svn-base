...
String calendarDate = reportHelper.getCalendarDate(WorkshopID, "instantQuizCondectedDate", CoordinatorID.toString());			
...
<html>
	<head>
		...
		<title>Instant Quiz Report</title>
	...
	<body class="ui-Mainbody" style="width:100%; height:100%; text-align: center;">
		...
		<table class="table1">
			<tr ><td >
				<div class="ui-header-text" ><h2>Instant Quiz Report</h2></div>
			</td></tr>
		</table>
		<table class="table1" style="margin-top:8px;" border="1" >
			<tr ><td width="22%">
				<label style="margin-left:35px;">Date :</label>&nbsp;
				<input type="text" id="quiz_datepicker"	style="width: 85px;" onchange="fillInstantQuizTime()" ><br><br>
				<div id="quiztime" style="display: none;">
					<select name="quiztimeselect" id="quiztimeselect" onchange="setInstantQuizTime()">
						<option value="Time Stamp">Time Stamp</option>
					</select>					
				</div> <br/>
				<input type="hidden" style="display: none;" id="hide_qid" name="hide_qid" />
				<input type="hidden" style="display: none;" id="hide_qts" name="hide_qts" />
				<span style=""></span><input type="radio" checked="checked" value="InstantQuizResponse" name="instantquizrpt"/>
				<label>Instant Quiz Response</label><br><br>				
				<button class="ui-conductquiz-button" id="quizreport_btn" type="button" style="margin-left: 50px;" onclick="instantQuizReport('<%=dates%>')">
