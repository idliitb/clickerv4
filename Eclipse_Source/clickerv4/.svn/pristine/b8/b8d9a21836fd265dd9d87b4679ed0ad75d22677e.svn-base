...
function studentInfo(id){
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			var studdetails = "<table border='1px' style='margin:auto;border-radius:5px;''> <tr> <td>Student Name </td><td> " + stud[0];
				studdetails += "</td></tr><tr><td> Year of Joining </td><td> " + stud[1];
				studdetails += "</td></tr><tr><td>Teaching Assistant </td><td> " + stud[2] + "</td></tr><tr><td> Mac Address </td><td> " + stud[3] + "</td></tr></table>";
				document.getElementById("studentinfo").innerHTML = studdetails;
		}
	};
	xmlhttp.open("GET", "../../jsp/report/reporthelper.jsp?helpcontent=studentinfo&id=" + id,	true);
	xmlhttp.send();	
}
...
function fillQuiz(courseID, date, dates) {
	...
	xmlhttp.open("GET",	"../../jsp/report/reporthelper.jsp?helpcontent=quizname&courseID="+ encodeURIComponent(courseID) +"&date="+date, true);
	xmlhttp.send();
}
...
function fillQuizTime(quizid) {
	...
	var dateArray = date.split("/");
	date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	xmlhttp.open("GET", "../../jsp/report/reporthelper.jsp?helpcontent=quiztime&qid="+ quizid + "&qdate="+date, true);
	xmlhttp.send();	
}
...
function fillInstantQuizTime() {
	...
	var dateArray = date.split("/");
	date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	xmlhttp.open("GET", "../../jsp/report/reporthelper.jsp?helpcontent=instantquiztime&qdate="+date, true);
	xmlhttp.send();	
}
...
function fillPollTime() {
	...
	var dateArray = date.split("/");
	date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	xmlhttp.open("GET", "../../jsp/report/reporthelper.jsp?helpcontent=polltime&qdate="+date, true);
	xmlhttp.send();	
}
...
function quizReport(dates) {
	...
	if(reportname=="QuizDetail"){
		...
		xmlhttp.open("GET", "../../Chart?report=quizreport&qid="+ quizid + "&qts="+qts + "&charttype="+reportname+"Chart", false);
		xmlhttp.send();
	}
}
...
function generateQuizReport(reportname){
	...
	var quizid = quizname_op.options[quizname_op.selectedIndex].value;
	var qts = document.getElementById("hide_qts").value;
	xmlhttp.open("GET", "../../Report?report=quizreport&qid="+ quizid + "&qts="+qts + "&reportname="+reportname, true);
	xmlhttp.send();
}
...
function instantQuizReport(dates) {
	...
	xmlhttp.open("GET", "../../Chart?charttype=InstantQuizResponseChart&qid="+ qid, false);
	xmlhttp.send();	
}
...
function pollReport(dates) {
	...
	xmlhttp.open("GET", "../../Chart?charttype=PollResponseChart&pid="+ pid, false);
	xmlhttp.send();	
}
...
function generateInstantQuizReport(){
	...
	var qid = document.getElementById("hide_qid").value;
	var qts = document.getElementById("hide_qts").value;
	xmlhttp.open("GET", "../../Report?report=instantquizreportnew&qid="+ qid + "&qts="+qts, true);
	xmlhttp.send();
}
...
function generatePollReport(){
	...
	xmlhttp.open("GET", "../../Report?report=pollreport&pid="+ pid + "&pts="+pts, true);
	xmlhttp.send();
}
...
function fillAttenDetail(courseID, date) {
	...
	var dateArray = date.split("/");
	date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	xmlhttp.open("GET",	"../../jsp/report/reporthelper.jsp?helpcontent=atteninfo&cid="+ encodeURIComponent(courseID) + "&date="+date, true);	
	xmlhttp.send();	
}
...
function coruseRport(cid){
	...
	if(reportname == "StudentQuery"){
		...
		xmlhttp.open("GET", "../../Chart?charttype=Attendance&cid=" + encodeURIComponent(cid) + "&ats=" + atttime, false);
		xmlhttp.send();
	}else{
		generateCourseReport(reportname);
	}
...
function generateCourseReport(reportname){
	...
	if(reportname=="Attendance"){
		...
		date = document.getElementById("studentQuery_datepicker").value;		
		var dateArray = date.split("/");
		date = dateArray[2] + "-" + dateArray[0] +"-"+ dateArray[1];
	}
	xmlhttp.open("GET", "../../Report?report=corusereport&ats="+ atttime + "&reportname="+reportname + "&date=" +date, true);
	xmlhttp.send();
}